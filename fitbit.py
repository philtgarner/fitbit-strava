...
def getHeartRateHistory(access_token: str, days: int = 30):
    # Get some data
    endpoint = f"https://api.fitbit.com/1/user/-/activities/heart/date/today/{days}d.json"
    headers = {"Authorization": f"Bearer {access_token}"}
    return requests.get(endpoint, headers=headers).json()

def getHeartRateDetailed(access_token: str, day: datetime = None):
...
def getSleepHistory(access_token: str, end: datetime = None, duration: int = 30):
    if end is None:
